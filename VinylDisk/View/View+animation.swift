...
extension ViewController{
    ...
    @objc func startRotate() {
        UIView.animate(withDuration: 0.0, delay: 0, options: .curveLinear) {
            self.rotatedView.transform = CGAffineTransform(rotationAngle: self.degree.degree)
        } completion: { [weak self] finished in
            guard let self = self
            else { return }
            self.degree.degree += self.cgDegree.cgDegree * CGFloat(Double(self.currentSpeed) / 180)
        }
    }
